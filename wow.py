...
def find_words(n_words=5, words='AEIOU', mask="", permutations=True, combinations=False):
    """
    ...
    if mask:  # mask not empty
        ...
        list_of_words.extend(''.join(permutation)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--permutation',
                        dest='permutation', action='store_true')
    parser.add_argument('-c', '--combination',
                        dest='combination', action='store_true')

    args = parser.parse_args()
    print(' '.join(find_words(args.number, args.words,
          args.mask, args.permutation, args.combination)))
